...
def mock_smart_memory():
    """Create a mock SmartMemory instance with a backend-like _graph."""
    sm = Mock()
    # _backend = getattr(sm._graph, "backend", sm._graph)
    # Since sm._graph.backend doesn't exist by default, _backend resolves to sm._graph
    sm._graph.get_node = Mock(return_value=None)
    sm._graph.get_neighbors = Mock(return_value=[])
    # Remove the 'backend' attribute so getattr falls back to sm._graph itself
    del sm._graph.backend
    return sm
...
class TestSimilarityGraphTraversal:
    ...
    def test_initialization(self, mock_smart_memory):
        ...
        with patch('smartmemory.stores.vector.vector_store.VectorStore'):
            ...
            # _backend resolves to sm._graph (since sm._graph has no .backend attr)
            assert ssg._backend == mock_smart_memory._graph
    ...
    def test_get_memory_item_success(self, ssg_traversal, mock_smart_memory):
        """Test retrieving memory item via backend.get_node()."""
        # _backend.get_node returns a dict; _get_memory_item wraps it as MemoryItem
        mock_smart_memory._graph.get_node.return_value = {
            "item_id": "test_123",
        ...
        mock_smart_memory._graph.get_node.assert_called_once_with("test_123")

    def test_get_memory_item_not_found(self, ssg_traversal, mock_smart_memory):
        """Test retrieving non-existent memory item."""
        mock_smart_memory._graph.get_node.return_value = None
    ...
    def test_get_neighbors_graph_only(self, ssg_traversal, mock_smart_memory, mock_vector_store):
        ...
        # _backend.get_neighbors returns list of dicts
        mock_smart_memory._graph.get_neighbors.return_value = [{"item_id": "neighbor_1"}]
    ...
    def test_get_neighbors_excludes_visited(self, ssg_traversal, mock_smart_memory, mock_vector_store):
        ...
        mock_smart_memory._graph.get_neighbors.return_value = [{"item_id": "visited_node"}]
    ...
    def test_query_traversal_basic_flow(self, ssg_traversal, mock_smart_memory, mock_vector_store):
        ...
        # Mock anchor item via backend.get_node (returns dict)
        mock_smart_memory._graph.get_node.return_value = {
            "item_id": "anchor_123",
...
class TestSSGIntegration:
    ...
    def test_multi_hop_traversal_simulation(self, ssg_traversal, mock_smart_memory, mock_vector_store):
        ...
        # Mock items — backend.get_node returns dicts
        items = {
            'chunk_1': {"item_id": "chunk_1", "content": "First chunk", "memory_type": "semantic"},
            'chunk_2': {"item_id": "chunk_2", "content": "Second chunk", "memory_type": "semantic"},
            'chunk_3': {"item_id": "chunk_3", "content": "Third chunk", "memory_type": "semantic"},
        }

        mock_smart_memory._graph.get_node.side_effect = lambda item_id: items.get(item_id)

        # Mock neighbors: chunk_1 -> chunk_2 -> chunk_3  (dicts, not Mock objects)
        def get_neighbors_side_effect(chunk_id):
            if chunk_id == 'chunk_1':
                return [{"item_id": "chunk_2"}]
            elif chunk_id == 'chunk_2':
                return [{"item_id": "chunk_3"}]
            return []
