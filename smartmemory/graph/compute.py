...
class GraphComputeLayer:
    ...
    def sync_remove_edge(self, source_id: str, target_id: str, edge_type: str | None = None) -> None:
        ...
        if edge_type is not None:
            ...
            # Remove all edges between the pair (no edge_type filter in SQL DELETE)
            while self._graph.has_edge(source_id, target_id):
                # MultiDiGraph.remove_edge without key removes one arbitrary edge;
    ...
    def nx_graph(self) -> nx.MultiDiGraph:
        """The underlying NetworkX MultiDiGraph — use for algorithms that need raw access."""
        return self._graph
