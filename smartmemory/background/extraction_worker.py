...
def process_extract_job(
    ...
    # NOTE: Each write phase auto-commits individually. We deliberately avoid
    # wrapping all phases in a single transaction because a rollback would
    # discard ALL Tier 2 writes (entities, relations, patterns) even if only
    # one phase failed. Individual auto-commits are safer: a failed relation
    # write cannot cascade and destroy successfully written entities.
