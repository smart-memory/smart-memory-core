...
#### CORE-SEARCH-1 — Query Decomposition into Facets (COMPLETE)
...
- **Auto-detect LLM API key (1d)**: `PipelineConfig.lite(llm_enabled=None)` auto-detects `OPENAI_API_KEY`/`GROQ_API_KEY`. Users with an API key get 85%+ R-F1 instead of 65% ruler-only. Explicit `llm_enabled=True/False` override available.
...
- **71+ tests** across 7+ files covering all 6 sub-features.
...
- `IndexResult.entities` field in `smartmemory/code/models.py` — list of all parsed `CodeEntity` objects produced during indexing. Used by `SmartMemory.ingest_code()` to pass entities to `seed_patterns_from_code()`.
- `IndexResult.embeddings_generated` field — count of embeddings successfully written to the vector store.
- `SmartMemory.ingest_code(directory, repo, commit_hash=None, exclude_dirs=None)` in `smartmemory/smart_memory.py` — public API for code indexing. Constructs `CodeIndexer`, calls `index()`, auto-detects `commit_hash` via `git rev-parse HEAD` when not provided, and calls `seed_patterns_from_code()` with the entity list.
- `SmartMemory.seed_patterns_from_code(entities)` — seeds the `PatternManager` with `{entity.name: entity.entity_type}` from a code index so that subsequent `ingest()` calls recognise code symbols (class names, function names) as typed entities.
- `PatternManager.add_patterns(patterns)` in `smartmemory/ontology/pattern_manager.py` — merges a `dict[str, str]` of name→type into the in-memory pattern cache and persists each entry to the ontology graph via `add_entity_pattern()` with `source="code_index"`. Filters short names (< 2 chars) and common word blocklist. Returns count of newly accepted patterns.
- `MemoryItem.display_text` property in `smartmemory/models/memory_item.py` — returns `f"{name} ({file_path})"` for `memory_type="code"` items (which have empty `content`), falls through to `content` for all other types. Provides a single safe entrypoint for any display/MCP rendering logic that needs to handle both code and memory results.
- `import subprocess` added to module-level imports in `smartmemory/smart_memory.py` for git invocation in `ingest_code()`.
- 23 new unit tests in `tests/unit/test_code_dev2_embeddings.py` covering: embedding generation per entity, composite text construction, empty/failed entity handling, `IndexResult.embeddings_generated` population, `ingest_code()` seed call, no-entity no-seed path, commit hash auto-detection, explicit commit hash bypass, `seed_patterns_from_code()` forwarding, empty/nameless entity filtering, ontology-disabled no-op, `PatternManager.add_patterns()` acceptance/deduplication/blocklist/persistence, and `MemoryItem.display_text` for code/non-code items.

**Vector collection routing confirmed:** `VectorStore()` with no `collection_name` argument resolves to `semantic_memory` (the default in `vector_store.py:130`). `_search_with_vector_embeddings()` in `graph/core/search.py` queries that same collection via `VectorStore()`. Code node embeddings therefore land in the correct collection and appear in `SmartMemory.search()` results with no search path changes needed — confirmed by the S1 spike result in `docs/features/CODE-DEV/design.md`.

#### CODE-DEV-7 — Polyglot Parser: TypeScript/JavaScript + React

- `smartmemory/code/ts_parser.py` — new tree-sitter-based parser for `.ts`, `.tsx`, `.js`, and `.jsx` files. Extracts modules, classes, functions, React components, and React hooks. Uses lazy-initialised language singletons for the three grammars (TSX, TypeScript, JavaScript).
- **React component detection** — arrow functions and named functions whose body contains a `jsx_element`, `jsx_self_closing_element`, or `jsx_fragment` tree-sitter node are classified as `entity_type="component"`.
- **Hook detection** — functions and arrow consts whose name starts with `use` followed by an uppercase letter (`useAuth`, `useMemory`, etc.) are classified as `entity_type="hook"`. `useless`, `user`, etc. are NOT hooks.
- `collect_ts_files()` in `ts_parser.py` — mirrors `collect_python_files()` for TS/JS extensions; excludes `node_modules`, `dist`, `build`, `.next`, `out`, `coverage`, and other standard JS build artifact directories.
- `parse_file()` dispatch function in `smartmemory/code/parser.py` — routes `.py` files to `CodeParser` and `.ts/.tsx/.js/.jsx` files to `TSParser`; returns an error `ParseResult` for unsupported extensions.
- `CodeIndexer.index(languages=...)` parameter — defaults to `["python"]` for backwards compatibility. Pass `["python", "typescript"]` to also collect and parse TS/JS files via the language-aware `parse_file()` dispatch.
- Version pins added to `requirements.txt` and `pyproject.toml` (`polyglot` optional extra): `tree-sitter==0.25.2`, `tree-sitter-typescript==0.23.2`, `tree-sitter-javascript==0.25.0`. Versions are pinned because the tree-sitter Python API changed at v0.22 and grammar packages are not cross-version compatible.
- 30 new unit tests in `tests/unit/test_ts_parser.py`: entity type extraction (function, hook, component, class, method), hook detection edge cases, `item_id` format and determinism, language dispatch routing for all four TS/JS extensions and `.py`, DEFINES/IMPORTS relations, `collect_ts_files()` with directory exclusion, and error handling for unsupported extensions and empty files.
- **Validated against real production code**: `App.tsx` (89 KB, 48 entities, 634 relations extracted, zero parse errors).

#### CODE-DEV-5 — Cross-File Call Resolution

- `ImportSymbol` dataclass in `smartmemory/code/models.py` — records a single imported name with `importing_file`, `local_name`, `module_path`, `original_name`, and `is_module_import` fields. Populated by `CodeParser._extract_import()` and consumed by `CodeIndexer`.
- `ParseResult.import_symbols` field — list of `ImportSymbol` records produced during parsing; carries structured import data across the parse→index boundary without modifying the graph edge schema.
- `SymbolTable` class in `smartmemory/code/indexer.py` — two-phase cross-file symbol resolution. Phase 1 registers all `CodeEntity` objects by their dotted module path. Phase 2 binds each `ImportSymbol`'s `local_name` to the resolved `item_id`. Supports: `from foo import Bar`, `from foo import Bar as B`, `import foo as f`, and attribute-call resolution `f.Bar`. Scoped per importing file to prevent cross-file name leakage.
- `CodeIndexer._build_symbol_table()` — constructs a `SymbolTable` from all parsed entities and import symbols after the first parsing pass.
- `CodeIndexer._resolve_cross_file_calls()` — second resolution pass on all `CALLS` relations. For each CALLS edge, derives the caller's file from `source_id`, looks up the `callee` property (raw call name) in the symbol table, and replaces the same-file guess with the correct cross-file `item_id`. Unresolvable edges fall through unchanged and are filtered by the existing `entity_ids` membership check.
- `CodeParser._extract_calls()` now stores the raw callee name in `properties["callee"]` so the indexer's resolution pass can look it up without re-parsing.
- `IndexResult.entities` populated by `CodeIndexer.index()` — exposes the full entity list for downstream consumers (e.g. `seed_patterns_from_code()` in CODE-DEV-4).
- 8 new unit tests in `tests/unit/test_code_parser.py` across `TestSymbolTable` and `TestCrossFileCallResolution`: symbol table built from imports, aliased imports, module imports with attribute-call resolution, unknown-name returns None, per-file scope isolation, end-to-end cross-file call resolution, unresolvable calls still filtered, and import symbol population by parser.
- **Review loop fix** — `CodeParser._get_call_name()` updated to return the full dotted callee name for attribute calls (e.g. `f.bar.baz()` → `"f.bar.baz"` instead of `"baz"`). This wires module-import style calls (`import lib as f; f.process()`) through to the symbol table's pre-registered `"f.process"` key, completing end-to-end cross-file call resolution for all import styles. Private-method filter updated to check the final component (`callee.split(".")[-1].startswith("_")`) so private methods on objects are still excluded.
- 5 additional coverage tests added to `TestCrossFileCallResolution`: module-import alias end-to-end resolution, wildcard import graceful ignore, circular imports no-infinite-loop, empty repo no-crash, and module-with-only-module-entity.
